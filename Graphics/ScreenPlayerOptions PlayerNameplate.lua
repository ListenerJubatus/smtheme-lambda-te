...
local t = Def.ActorFrame {
	LoadActor(THEME:GetPathB("_frame","3x1"),"rounded fill", 250-16) .. {
		OnCommand=cmd(diffuse,PlayerColor(PlayerNumber);diffusealpha,1);
	};
	LoadFont("Common Condensed") .. {
		Text=ToEnumShortString(PlayerNumber);
		Name="PlayerShortName",
		InitCommand=cmd(x,-127;maxwidth,32;zoom,0.75;),
		OnCommand=cmd(diffuse,ColorLightTone(PlayerColor(PlayerNumber));)
	},
	LoadFont("_overpass 36px") .. {
		Text=bpm_text;
		Name="BPMRangeOld",
		InitCommand=cmd(x,-60;maxwidth,88/bpm_text_zoom),
		OnCommand=cmd(zoom,bpm_text_zoom;diffuse,ColorLightTone(PlayerColor(PlayerNumber));)
	},
	...
	LoadFont("_overpass 36px") .. {
		...
		BPMWillNotChangeCommand=cmd(stopeffect;diffuse,ColorLightTone(PlayerColor(PlayerNumber));),
		BPMWillChangeCommand=cmd(diffuseshift;effectcolor1,Color.White;effectcolor2,ColorLightTone(PlayerColor(PlayerNumber));),
